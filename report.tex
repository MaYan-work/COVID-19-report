...
\KeywordTok{options}\NormalTok{(}\DataTypeTok{digits =} \DecValTok{2}\NormalTok{) }\CommentTok{# format decimal places to 2}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{filter}\NormalTok{(countryterritoryCode }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"SGP"}\NormalTok{, }\StringTok{"USA"}\NormalTok{),}
\NormalTok{         month }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{3}\OperatorTok{:}\DecValTok{5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{"New Cases and Deaths (per million)"}\NormalTok{) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{select}\NormalTok{(date, month,  week, country, cases_per_mil, deaths_per_mil, deaths_prop, yday) }\OperatorTok{%>%}
...
\StringTok{  }\KeywordTok{group_by}\NormalTok{(country, week, month) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{cases_per_week =} \KeywordTok{sum}\NormalTok{(cases_per_mil),}
            ...
            \DataTypeTok{deaths_prop =} \KeywordTok{mean}\NormalTok{(deaths_prop, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)) }\CommentTok{# % of deaths per week}

\KeywordTok{kable}\NormalTok{(summary_sgp_usa_week, }\DataTypeTok{caption =} \StringTok{"Total Cases and Deaths, and Proportion of Deaths per week"}\NormalTok{) }\OperatorTok{%>%}
